...
void testEliminaCurent() {
	...
	TElem deleted = im.elimina();
	assert(elem.first == 3);
	assert(elem.second == 6);

	assert(elem.first == deleted.first);
	assert(elem.second == deleted.second);

	assert(im.element().first == 2);
	assert(im.element().second == 5);
