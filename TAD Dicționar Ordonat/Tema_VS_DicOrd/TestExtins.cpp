...
/*void mytest() {
	...
	while (it.valid())
	{
		...
		//cout << abs(it.element().first) % MAX << endl;
		it.urmator();
	}
}*/
...
void populeazaDOSh(DO& doc, int cMin, int cMax, int shift) {
	...
	for (int i = 0; i < n; i++) {
		TCheie ch;
		ch = doc.adauga(chei[i], chei[i] - shift);
	}
}
...
void testAdaugaSiCauta(Relatie r) {
	...
	it.prim();
	if (it.valid()) {
		TElem cPrec = it.element();
		assert(d.cauta(cPrec.first) != NULL_TVALOARE);
		it.urmator();
		while (it.valid()) {
			TElem c = it.element();
			...
			assert(r(cPrec, c));
			assert(d.cauta(c.first) != NULL_TVALOARE);
			cPrec = c;
			it.urmator();
		}
	}
...
void testIterator(Relatie r) {
	...
	TElem cPrec = itD.element();
	for (int i = 1; i < 100; i++) {
		assert(cPrec.first == itD.element().first);
	}
	itD.urmator();
	while (itD.valid()) {
		TElem c = itD.element();
		assert(cMin <= c.first && c.first <= cMax);
		assert(d.cauta(c.first) != NULL_TVALOARE);
		assert(r(cPrec, c));
		cPrec = c;
		itD.urmator();
...
void testDiferenta() {
	...
	assert(d.diferentaValoareMaxMin() == 9);
	d.sterge(1);
	assert(d.diferentaValoareMaxMin() == 3);
	d.sterge(3);
	assert(d.diferentaValoareMaxMin() == 0);
	d.sterge(4);
	assert(d.diferentaValoareMaxMin() == -1);
}
